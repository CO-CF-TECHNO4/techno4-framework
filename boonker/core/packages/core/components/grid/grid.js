    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
        ...
        if ($prevResizableEl.length) {
          ...
          gapSize = parseFloat($prevResizableEl.css(isRow ? '--t4-grid-row-gap' : '--t4-grid-gap'));
        }
        if ($nextResizableEl.length) {
          ...
          if (!$prevResizableEl.length) {
            ...
            gapSize = parseFloat($nextResizableEl.css(isRow ? '--t4-grid-row-gap' : '--t4-grid-gap'));
          }
        }
      }
      isMoved = true;
      ...
      const gapAddSizeCSS = isRow ? `${itemsInFlow - 1} * var(--t4-grid-row-gap) / ${itemsInFlow}` : '(var(--t4-cols-per-row) - 1) * var(--t4-grid-gap) / var(--t4-cols-per-row)';
