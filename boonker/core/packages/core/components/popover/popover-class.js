...
class Popover extends Modal {
  constructor(app, params) {
    ...
    if (popover.params.backdrop && popover.params.backdropEl) {
      ...
      if (popover.params.backdropUnique || forceBackdropUnique) {
        ...
        $backdropEl[0].t4PopoverRef = popover;
    ...
    function handleClick(e) {
      ...
      if ($target.closest(popover.el).length === 0) {
        if (popover.params.closeByBackdropClick && popover.params.backdrop && popover.backdropEl && popover.backdropEl === target && touchStartTarget === target) {
          ...
          const isAnotherPopoverBackdrop = $target.hasClass('popover-backdrop-unique') && target.t4PopoverRef !== popover || $target.hasClass('popover-backdrop') && target !== popover.backdropEl;
  ...
  resize() {
    ...
    let targetOffsetTop;
    let safeAreaTop = parseInt($('html').css('--t4-safe-area-top'), 10);
    let safeAreaLeft = parseInt($('html').css('--t4-safe-area-left'), 10);
    let safeAreaRight = parseInt($('html').css('--t4-safe-area-right'), 10);
