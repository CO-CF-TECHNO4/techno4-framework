...
class Searchbar extends FrameworkClass {
  constructor(app, params) {
    ...
    if ($el[0].t4Searchbar) return $el[0].t4Searchbar;
    $el[0].t4Searchbar = sb;
    let $pageEl;
    ...
    if ($el.parents('.page').length > 0) {
      ...
      if (!$pageEl.length) {
        ...
        if ($currentPageEl[0] && $currentPageEl[0].t4Page && $currentPageEl[0].t4Page.navbarEl === $navbarEl[0]) {
  ...
  search(query, internal) {
    ...
    let vlQuery;
    if (isVirtualList) {
      sb.virtualList = $searchContainer[0].t4VirtualList;
      if (query.trim() === '') {
  ...
  destroy() {
    ...
    sb.detachEvents();
    if (sb.$el[0]) {
      sb.$el[0].t4Searchbar = null;
      delete sb.$el[0].t4Searchbar;
    }
    deleteProps(sb);
  }
}
