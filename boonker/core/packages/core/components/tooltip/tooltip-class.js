...
class Tooltip extends techno4Class {
  constructor(app, params) {
    ...
    if ($targetEl[0] && $targetEl[0].t4Tooltip && !tooltip.params.delegated) return $targetEl[0].t4Tooltip;
    ...
    if ($targetEl[0]) $targetEl[0].t4Tooltip = tooltip;
    const touchesStart = {};
    let isTouched;
  ...
  destroy() {
    ...
    tooltip.$el.remove();
    if (tooltip.$targetEl[0]) delete tooltip.$targetEl[0].t4Tooltip;
    tooltip.detachEvents();
    deleteProps(tooltip);
