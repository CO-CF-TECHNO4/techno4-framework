...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.techno4 = factory());
...
`;
        ...
        events.split(' ').forEach(event => {
          if (typeof handler === 'undefined') {
            ...
            self.eventsListeners[event].forEach((eventHandler, index) => {
              if (eventHandler === handler || eventHandler.t4proxy && eventHandler.t4proxy === handler) {
    ...
    function loadModule(moduleToLoad) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (typeof moduleToLoad === 'string') {
          ...
          if (moduleToLoad.indexOf('.') < 0 && matchNamePattern && matchNamePattern[0].length === moduleToLoad.length) {
            if (!app || app && !app.params.lazyModulesPath) {
              reject(new Error('techno4: "lazyModulesPath" app parameter must be specified to fetch module by name'));
              return;
            }
        ...
        if (moduleFunc) {
          ...
          if (!module) {
            reject(new Error("techno4: Can't find techno4 component in specified component function"));
            return;
          }
        ...
        if (modulePath) {
          ...
          const scriptLoad = new Promise((resolveScript, rejectScript) => {
            techno4.request.get(modulePath, scriptContent => {
              const callbackId = id();
              ...
              if (!module) {
                rejectScript(new Error(`techno4: Can't find techno4 component in ${modulePath} file`));
                return;
              }
          ...
          const styleLoad = new Promise(resolveStyle => {
            techno4.request.get(modulePath.replace('.lazy.js', app.rtl ? '.rtl.css' : '.css').replace('.js', app.rtl ? '.rtl.css' : '.css'), styleContent => {
    ...
    class techno4 extends techno4Class {
      ...
      // eslint-disable-next-line
      loadModule() {
        return techno4.loadModule(...arguments);
      }

      // eslint-disable-next-line
      loadModules() {
        return techno4.loadModules(...arguments);
      }
    ...
    function SwipeBack(r) {
      ...
      const transformOriginTitleLarge = app.rtl ? 'calc(100% - var(--t4-navbar-large-title-padding-left) - var(--t4-safe-area-left)) center' : 'calc(var(--t4-navbar-large-title-padding-left) + var(--t4-safe-area-left)) center';
            ...
            if (isLeft) {
              ...
              $navEl.find('.back span').each(subNavEl => {
                els.push({
                  ...
                  transform: progress => `translateX(calc(${progress} * (var(--t4-navbarTitleLargeOffset) - var(--t4-navbarLeftTextOffset)))) translateY(calc(${progress} * (var(--t4-navbar-large-title-height) - var(--t4-navbar-large-title-padding-vertical) / 2))) scale(${1 + 1 * progress})`
                });
              });
              return;
            }
          ...
          if (isBg) {
            ...
            if (!fromLarge && !toLarge) {
              if (currentNavIsCollapsed) {
                ...
                el.transform = progress => `translateX(${100 * progress * inverter}%) translateY(calc(-1 * var(--t4-navbar-large-title-height)))`;
              } else {
            ...
            if (!fromLarge && toLarge) {
              el.className = 'ios-swipeback-navbar-bg-large';
              el.transform = progress => `translateX(${100 * progress * inverter}%) translateY(calc(-1 * ${1 - progress} * var(--t4-navbar-large-title-height)))`;
            }
            if (fromLarge && toLarge) {
              el.transform = progress => `translateX(${100 * progress * inverter}%)`;
            }
            if (fromLarge && !toLarge) {
              el.transform = progress => `translateX(${100 * progress * inverter}%) translateY(calc(-${progress} * var(--t4-navbar-large-title-height)))`;
            }
            return;
          }
            ...
            if ($navEl.hasClass('title-large')) {
              ...
              $navEl.find('.title-large-text').each(subNavEl => {
                els.push({
                  ...
                  transform: progress => `translateX(calc(${1 - progress} * (var(--t4-navbarLeftTextOffset) - var(--t4-navbarTitleLargeOffset)))) translateY(calc(${progress - 1} * var(--t4-navbar-large-title-height) + ${1 - progress} * var(--t4-navbar-large-title-padding-vertical))) scale(${0.5 + progress * 0.5})`
                });
              });
              return;
            }
          ...
          if (isBg) {
            ...
            if (!fromLarge && !toLarge) {
              if (previousNavIsCollapsed) {
                ...
                el.transform = progress => `translateX(${(-100 + 100 * progress) * inverter}%) translateY(calc(-1 * var(--t4-navbar-large-title-height)))`;
              } else {
                el.transform = progress => `translateX(${(-100 + 100 * progress) * inverter}%)`;
              }
            }
            if (!fromLarge && toLarge) {
              el.transform = progress => `translateX(${(-100 + 100 * progress) * inverter}%) translateY(calc(-1 * ${1 - progress} * var(--t4-navbar-large-title-height)))`;
            }
            if (fromLarge && !toLarge) {
              el.className = 'ios-swipeback-navbar-bg-large';
              el.transform = progress => `translateX(${(-100 + 100 * progress) * inverter}%) translateY(calc(-${progress} * var(--t4-navbar-large-title-height)))`;
            }
            if (fromLarge && toLarge) {
              el.transform = progress => `translateX(${(-100 + 100 * progress) * inverter}%)`;
            }
    ...
    function appRouterCheck(router, method) {
      if (!router.view) {
        throw new Error(`techno4: it is not allowed to use router methods on global app router. Use router methods only on related View, e.g. app.views.main.router.${method}(...)`);
      }
    }
    ...
    function forward(router, el, forwardOptions) {
      ...
      } else if (options.route && options.route.route && options.route.route.keepAlive && !$newPage[0].t4PageMounted) {
    ...
    function navigate(navigateParams, navigateOptions) {
      ...
      if (route.route && route.route.viewName) {
        ...
        if (!anotherView) {
          throw new Error(`techno4: There is no View with "${anotherViewName}" name that was specified in this route`);
        }
    ...
    function backward(router, el, backwardOptions) {
      ...
      if (masterDetailEnabled) {
        ...
        isDetail = !isMaster && masterPageEl && router.history.indexOf(options.route.url) > router.history.indexOf(masterPageEl.t4Page.route.url);
        if (!isDetail && !isMaster && masterPageEl && masterPageEl.t4Page && options.route.route.masterRoute) {
      ...
      if (isDetail && masterPageEl && masterPageEl.t4Page) {
        isDetailRoot = router.history.indexOf(options.route.url) - router.history.indexOf(masterPageEl.t4Page.route.url) === 1;
      }
    ...
    function back() {
      ...
      if (router.params.masterDetailBreakpoint > 0) {
        ...
        if ($previousMaster.length) {
          ...
          if (expectedPreviousPageRoute && $previousMaster[0].t4Page && expectedPreviousPageRoute.route === $previousMaster[0].t4Page.route.route) {
      ...
      if (!navigateOptions.force && $previousPage.length && !skipMaster) {
        if (router.params.browserHistory && $previousPage[0].t4Page && router.history[router.history.length - 2] !== $previousPage[0].t4Page.route.url) {
    ...
    class Router extends techno4Class {
          ...
          newNavEls.forEach(navEl => {
            ...
            if (navEl.isSliding) {
              if (navEl.isSubnavbar && newIsLarge) {
                // prettier-ignore
                $el[0].style.setProperty('transform', `translate3d(${offset * (1 - progress)}px, calc(-1 * var(--t4-navbar-large-collapse-progress) * var(--t4-navbar-large-title-height)), 0)`, 'important');
              } else {
          ...
          oldNavEls.forEach(navEl => {
            ...
            if (navEl.isSliding) {
              if (navEl.isSubnavbar && oldIsLarge) {
                $el.transform(`translate3d(${offset * progress}px, calc(-1 * var(--t4-navbar-large-collapse-progress) * var(--t4-navbar-large-title-height)), 0)`);
              } else {
      ...
      constructRouteUrl(route, _temp) {
        ...
        try {
          ...
          throw new Error(`techno4: error constructing route URL from passed params:\nRoute: ${path}\n${error.toString()}`);
        }
        if (query) {
      ...
      pageCallback(callback, pageEl, navbarEl, from, to, options, pageFromEl) {
        ...
        const keepAlive = $pageEl[0].t4Page && $pageEl[0].t4Page.route && $pageEl[0].t4Page.route.route && $pageEl[0].t4Page.route.route.keepAlive;
      ...
      restoreHistory() {
        const router = this;
        const window = getWindow();
        if (router.params.browserHistory && router.params.browserHistoryStoreHistory && window.localStorage && window.localStorage[`t4router-${router.view.id}-history`]) {
      ...
      getInitialUrl() {
        ...
        if ((window.cordova || window.Capacitor && window.Capacitor.isNative) && browserHistory && !browserHistorySeparator && !browserHistoryRoot && location.pathname.indexOf('index.html')) {
          // eslint-disable-next-line
          console.warn('techno4: wrong or not complete browserHistory configuration, trying to guess browserHistoryRoot');
    ...
    class View$1 extends techno4Class {
      constructor(app, el, viewParams) {
        ...
        if (!ssr) {
          ...
          if (!$el.length) {
            ...
            message += typeof el === 'string' ? `the selector "${el}" didn't match any element` : 'el must be an HTMLElement or Dom64 object';
        ...
        if (validUrl || $clickedLinkEl.hasClass('back')) {
          ...
          if (clickedLinkData.view && clickedLinkData.view === 'current') {
            ...
            if (!$clickedLinkEl.hasClass('back') && view && view.params.linksView) {
              if (typeof view.params.linksView === 'string') view = $(view.params.linksView)[0].t4View;else if (view.params.linksView instanceof View$1) view = view.params.linksView;
            }
          }
          if (!view) {
    ...
    const createCustomComponent = _ref => {
      ...
      t4.component.create(component, propsFromAttrs(data.attrs || {}, data.props || {}), {
        el: vnode.elm,
    ...
    const treeNodeToVNode = (treeNode, component, t4, initial, isRoot) => {
      ...
      const children = isCustomComponent(treeNode.type) ? [] : getChildren(treeNode, component, t4, initial);
      ...
      constructor(app, component, props, _temp) {
        ...
        const getRenderFuncion = componentResult => new Promise((resolve, reject) => {
          if (typeof componentResult === 'function') {
            ...
            reject(new Error('techno4: Component render function is not a "function" type. Didn\'t you forget to "return $render"?'));
          }
        });
        return new Promise((resolve, reject) => {
      ...
      size(el) {
        ...
        if (dynamicNavbar && app.theme === 'ios') {
            ...
            if (app.rtl) {
              ...
              if (router && router.params.iosAnimateNavbarBackIcon && left.find('.back .icon').length > 0) {
                if (left.find('.back .icon ~ span').length) {
                  ...
                  left.find('.back .icon ~ span')[0].t4NavbarRightOffset = rightOffset - left.find('.back .icon')[0].offsetWidth;
                }
              }
            }
          }
          ...
          if (subnavbar.length && (subnavbar.hasClass('sliding') || sliding)) {
            subnavbar[0].t4NavbarLeftOffset = app.rtl ? subnavbar[0].offsetWidth : -subnavbar[0].offsetWidth;
      ...
      initNavbarOnScroll(pageEl, navbarEl, needHide, needCollapse, needTransparent) {
        ...
        function calcScrollableDistance() {
          $pageEl.find('.page-content').each(pageContentEl => {
            pageContentEl.t4ScrollableDistance = pageContentEl.scrollHeight - pageContentEl.offsetHeight;
          });
        }
        ...
        function handleLargeNavbarCollapse(pageContentEl) {
          ...
          const scrollableDistance = Math.min(navbarTitleLargeHeight, pageContentEl.t4ScrollableDistance || navbarTitleLargeHeight);
    ...
    class TouchRipple$1 {
      constructor(app, $el, x, y) {
        ...
        ripple.$rippleWaveEl = $(`<div class="ripple-wave" style="width: ${diameter}px; height: ${diameter}px; margin-top:-${diameter / 2}px; margin-left:-${diameter / 2}px; left:${center.x}px; top:${center.y}px; --t4-ripple-transform: ${ripple.rippleTransform}"></div>`);
    ...
    class Modal$1 extends techno4Class {
    ...
    techno4.use([DeviceModule, SupportModule, UtilsModule, ResizeModule, RequestModule, TouchModule, ClicksModule, RouterModule, HistoryModule, ComponentModule, ServiceWorkerModule, StoreModule, Statusbar$1, View, Navbar$1, Toolbar$1, Subnavbar, TouchRipple, Modal]);

    return techno4;

}));
//# sourceMappingURL=techno4.js.map
