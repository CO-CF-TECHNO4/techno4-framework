...
import { Navbar, Page, Block, Button, theme, t4 } from 'techno4-react';
...
export default () => {
  ...
  const showToastBottom = () => {
    ...
    if (!toastBottom.current) {
      toastBottom.current = t4.toast.create({
  ...
  const showToastTop = () => {
    // Create toast
    if (!toastTop.current) {
      toastTop.current = t4.toast.create({
  ...
  const showToastCenter = () => {
    ...
    if (!toastCenter.current) {
      toastCenter.current = t4.toast.create({
        text: "I'm on center",
        position: 'center',
        closeTimeout: 2000,
      });
  ...
  const showToastIcon = () => {
    ...
    if (!toastIcon.current) {
      toastIcon.current = t4.toast.create({
        icon:
  ...
  const showToastLargeMessage = () => {
    ...
    if (!toastLargeMessage.current) {
      toastLargeMessage.current = t4.toast.create({
  ...
  const showToastWithButton = () => {
    ...
    if (!toastWithButton.current) {
      toastWithButton.current = t4.toast.create({
  ...
  const showToastWithCustomButton = () => {
    ...
    if (!toastWithCustomButton.current) {
      toastWithCustomButton.current = t4.toast.create({
  ...
  const showToastWithCallback = () => {
    ...
    if (!toastWithCallback.current) {
      toastWithCallback.current = t4.toast.create({
