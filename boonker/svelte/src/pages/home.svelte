<script>
  ...
  const onPageAfterIn = () => {
    ...
    if (t4.width >= 768) {
      t4router.navigate('/about/', { reloadDetail: true });
    }
  };
  ...
  onMount(() => {
    if (theme.aurora) {
      ...
      t4.on('resize', onResize);

      t4router.on('routeChange', (route) => {
        const url = route.url;
        if (!$el) return;
...
<Page class="page-home" {onPageAfterIn}>
  <Navbar large transparent sliding={false}>
    <NavLeft>
      <Link panelOpen="left" iconIos="t4:menu" iconAurora="t4:menu" iconMd="material:menu" />
    </NavLeft>
    <NavTitle sliding>techno4 Svelte</NavTitle>
    <NavRight>
      <Link
        searchbarEnable=".searchbar-components"
        iconIos="t4:search"
        iconAurora="t4:search"
        iconMd="material:search"
      />
    </NavRight>
    <NavTitleLarge>techno4 Svelte</NavTitleLarge>
    <Searchbar
  ...
  <List class="searchbar-hide-on-search">
    <ListItem title="About techno4" reloadDetail={theme.aurora} link="/about/">
      <i class="icon icon-t4" slot="media" />
    </ListItem>
  </List>
