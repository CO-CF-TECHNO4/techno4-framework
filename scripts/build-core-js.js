...
async function modular({ components }) {
  ...
  const coreSrc = fs.readFileSync(path.resolve(__dirname, '../src/core/techno4.js'), 'utf-8');
  const liteSrc = fs.readFileSync(
    path.resolve(__dirname, '../src/core/techno4-lite.js'),
    'utf-8',
  );
  ...
  const coreComponents = (content, isLite) => {
    return removeUMD(content)
      ...
      .replace(
        ...
        } $ as Dom64, request, utils, getDevice, getSupport, createStore };`,
      );
  };
  ...
  // Save core
  fs.writeFileSync(`${outputDir}/techno4.esm.js`, coreContent);
  // Save bundle
  fs.writeFileSync(`${outputDir}/techno4-bundle.esm.js`, bundleContent);

  // Save lite
  fs.writeFileSync(`${outputDir}/techno4-lite.esm.js`, liteContent);
  // Save lite bundle
  fs.writeFileSync(`${outputDir}/techno4-lite-bundle.esm.js`, liteBundleContent);
  ...
  return rollup({
    ...
    plugins: [
      replace({
        ...
        '//IMPORT_HELPERS': '',
        '//NAMED_EXPORT': '',
        'export { $ as Dom64, request, utils, getDevice, getSupport, createStore, $jsx };': '',
      }),
    ...
    .then((bundle) => {
      ...
      return bundle.write({
        strict: true,
        file: `${output}/techno4-bundle.js`,
        format: 'umd',
        name: 'techno4',
        sourcemap: env === 'production',
        sourcemapFile: `${output}/techno4-bundle.js.map`,
        banner,
      });
    })
    .then(async (bundle) => {
      ...
      const minified = await minify(result.code, {
        sourceMap: {
          ...
          filename: env === 'production' ? 'techno4-bundle.min.js' : undefined,
          url: `techno4-bundle.min.js.map`,
        },
        output: {
          preamble: banner,
        },
      ...
      fs.writeFileSync(`${output}/techno4-bundle.min.js`, minified.code);
      fs.writeFileSync(`${output}/techno4-bundle.min.js.map`, minified.map);
    })
    .catch((err) => {
      console.log(err);
    });
...
async function umdCore() {
  ...
  return rollup({
    ...
    .then((bundle) => {
      ...
      return bundle.write({
        strict: true,
        file: `${output}/techno4.js`,
        format: 'umd',
        name: 'techno4',
        sourcemap: env === 'production',
        sourcemapFile: `${output}/techno4.js.map`,
        banner,
      });
    })
    .then(async (bundle) => {
      ...
      const minified = await minify(result.code, {
        sourceMap: {
          content: env === 'production' ? result.map : undefined,
          filename: env === 'production' ? 'techno4.min.js' : undefined,
          url: `techno4.min.js.map`,
        },
        output: {
          preamble: banner,
        },
      ...
      fs.writeFileSync(`${output}/techno4.min.js`, minified.code);
      fs.writeFileSync(`${output}/techno4.min.js.map`, minified.map);
    })
    .catch((err) => {
      console.log(err);
    });
