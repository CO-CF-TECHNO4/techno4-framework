...
const intro = `
function techno4ComponentLoader(techno4, techno4AutoInstallComponent) {
  if (typeof techno4AutoInstallComponent === 'undefined') {
    techno4AutoInstallComponent = true;
  }
  var $ = techno4.$;
  var Utils = techno4.utils;
  var getDevice = techno4.getDevice;
  var getSupport = techno4.getSupport;
  var techno4Class = techno4.Class;
  var Modal = techno4.Modal;
  var ConstructorMethods = techno4.ConstructorMethods;
  var ModalMethods = techno4.ModalMethods;
  var $jsx = techno4.$jsx;
`;

const install = `
  if (techno4AutoInstallComponent) {
    if (techno4.prototype.modules && techno4.prototype.modules[COMPONENT.name]) {
      return;
    }
    techno4.use(COMPONENT);
    if (techno4.instance) {
      techno4.instance.useModuleParams(COMPONENT, techno4.instance.params);
      techno4.instance.useModule(COMPONENT);
    }
  }
  return COMPONENT;
`;
...
async function buildLazyComponentsLess(components, rtl, cb) {
  ...
  const mainLess = fs
    .readFileSync(path.resolve(__dirname, '../src/core/techno4.less'))
    .split('\n')
    ...
    .then(() => {
      ...
      filesToProcess.forEach(async (fileName) => {
        ...
        if (fileName.indexOf('swiper') >= 0) {
          fileContent = fileContent
            .replace('var getDevice = techno4.getDevice;', '')
            .replace('var getSupport = techno4.getSupport;', '');
        }
        fileContent = fileContent
        ...
        fileContent = `(${fileContent}(techno4, typeof techno4AutoInstallComponent === 'undefined' ? undefined : techno4AutoInstallComponent))`;
