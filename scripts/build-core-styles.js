...
function copyLess(config, components, cb) {
  ...
  // Core LESS
  let lessContent = fs.readFileSync(path.resolve(__dirname, '../src/core/techno4.less'));
  lessContent = `${banner}\n${lessContent}`;
  ...
  fs.writeFileSync(`${output}/techno4.less`, lessContent);
  ...
  fs.writeFileSync(`${output}/techno4-bundle.less`, lessBundleContent);
...
async function buildBundle(config, components, themes, rtl, cb) {
  ...
  const includeDarkTheme = config.darkTheme;
  const includeLightTheme = config.lightTheme;
  const outputFileName = `techno4-bundle${rtl ? '-rtl' : ''}`;
  const output = `${getOutput()}/core`;
...
async function buildCore(themes, rtl, cb) {
  ...
  // Write file
  fs.writeFileSync(`${output}/techno4${rtl ? '-rtl' : ''}.css`, `${banner}\n${cssContent}`);
  ...
  // Write file
  fs.writeFileSync(
    `${output}/techno4${rtl ? '-rtl' : ''}.min.css`,
    `${banner}\n${minifiedContent}`,
  );
