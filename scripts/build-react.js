...
async function buildReact(cb) {
  ...
  const pluginContent = fs
    .readFileSync('src/react/techno4-react.js', 'utf-8')
    .replace('// IMPORT_COMPONENTS', componentImports.join('\n'))
    .replace('// EXPORT_COMPONENTS', `export { ${componentExports.join(', ')} }`);

  fs.writeFileSync(`${buildPath}/react/techno4-react.js`, pluginContent);

  await exec.promise(
    `MODULES=esm npx babel --config-file ./babel-react.config.js src/react --out-dir ${buildPath}/react --ignore "src/react/techno4-react.js","*.ts"`,
  );

  const esmContent = fs.readFileSync(`${buildPath}/react/techno4-react.js`, 'utf-8');

  fs.writeFileSync(
    `${buildPath}/react/techno4-react.js`,
    `${bannerReact.trim()}\n${esmContent}`,
  );
