...
async function buildSvelte(cb) {
  ...
  const pluginContent = fs
    .readFileSync('src/svelte/techno4-svelte.js', 'utf-8')
    .replace('// IMPORT_COMPONENTS', componentImports.join('\n'))
    .replace('// EXPORT_COMPONENTS', `export { ${componentExports.join(', ')} }`);

  fs.writeFileSync(`${buildPath}/svelte/techno4-svelte.js`, pluginContent);

  await exec.promise(
    `MODULES=esm npx babel --config-file ./babel-svelte.config.js src/svelte --out-dir ${buildPath}/svelte --ignore "src/svelte/techno4-svelte.js","*.svelte"`,
  );
  ...
  const esmContent = fs.readFileSync(`${buildPath}/svelte/techno4-svelte.js`, 'utf-8');

  fs.writeFileSync(
    `${buildPath}/svelte/techno4-svelte.js`,
    `${bannerSvelte.trim()}\n${esmContent}`,
  );
