...
async function release() {
  ...
  const webTypes = require('../packages/vue/techno4-vue-web-types.json');
  webTypes.version = options.version;
  fs.writeFileSync(
    path.resolve(__dirname, '../packages/vue/techno4-vue-web-types.json'),
  ...
  // Build Production Boonker
  await exec.promise('npm run build-boonker:core');
  await exec.promise('npm run build-boonker:react');
  await exec.promise('npm run build-boonker:svelte');
  await exec.promise('npm run build-boonker:vue');
