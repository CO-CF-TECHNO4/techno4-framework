  ...
  constructor(app, params) {
    ...
    function handleTouchEnd() {
      ...
      if (timeDiff < 300 && diff > 10 && useBreakpoints) {
        ...
        if (direction === openDirection && absCurrentTranslate < absSwipeStepTranslate) {
          ...
          if (sheet.push && pushOffset) {
            sheet.$htmlEl[0].style.setProperty('--t4-sheet-push-scale', pushViewScale(pushOffset));
            $pushViewEl.css('border-radius', '');
          }
        }
        if (direction === closeDirection && absCurrentTranslate > absSwipeStepTranslate) {
          ...
          if (sheet.params.swipeToClose) {
            ...
            if (sheet.push && pushOffset) {
              sheet.$htmlEl[0].style.removeProperty('--t4-sheet-push-scale');
    ...
    sheet.setBreakpoint = (value) => {
      ...
      if (value === 1) {
        ...
        $el[0].style.setProperty('--t4-sheet-breakpoint', `${breakpointsTranslate[index]}px`);
    ...
    sheet.setSwipeStep = function setSwipeStep(byResize = true) {
      ...
      $el[0].style.setProperty('--t4-sheet-swipe-step', `${swipeStepTranslate}px`);
      if (!byResize) {
        $el.addClass('modal-in-swipe-step');
        sheet.emit('local::_swipeStep', true);
      }
    ...
    sheet.on('open', () => {
      ...
      if (sheet.push) {
        pushOffset = parseInt($el.css('--t4-sheet-push-offset'), 10);
        if (Number.isNaN(pushOffset)) pushOffset = 0;
        if (!pushOffset) pushOffset = app.theme === 'ios' ? 44 : 48;
        sheet.$htmlEl[0].style.setProperty('--t4-sheet-push-offset', `${pushOffset}px`);
        $el.addClass('sheet-modal-push');
        if (!useBreakpoints) {
          $el.addClass('sheet-modal-push-in');
    ...
    sheet.on('closed', () => {
      if (sheet.push && pushOffset) {
        $el.removeClass('sheet-modal-push-closing');
        sheet.$htmlEl[0].style.removeProperty('--t4-sheet-push-scale');
        sheet.$htmlEl[0].style.removeProperty('--t4-sheet-push-offset');
      }
    });
