...
const AreaChart = forwardRef((props, ref) => {
  ...
  const setTooltip = () => {
    ...
    if (!hasVisibleDataSets) {
      if (t4Tooltip.current && t4Tooltip.current.hide) t4Tooltip.current.hide();
      return;
    }
    ...
    if (currentIndex !== null && !t4Tooltip.current) {
      ...
      if (t4Tooltip.current && t4Tooltip.current.show) {
        t4Tooltip.current.show();
      }
      return;
    }
    if (!t4Tooltip.current || !t4Tooltip.current.hide || !t4Tooltip.current.show) {
      return;
    }
    if (currentIndex !== null) {
      t4Tooltip.current.setText(formatTooltip());
...
AreaChart.displayName = 't4-area-chart';
