...
/* dts-imports
import { Calendar, ColorPicker, TextEditor } from 'techno4/types';
*/
/* dts-props
  id?: string | number;
  className?: string;
...
const Input = forwardRef((props, ref) => {
  ...
  const t4Calendar = useRef(null);
  const t4ColorPicker = useRef(null);
  ...
  const validateInput = () => {
    if (!t4 || !inputElRef.current) return;
  ...
  const onMount = () => {
    t4ready(() => {
      ...
      if (type === 'datepicker') {
        t4Calendar.current = t4.calendar.create({
      ...
      if (type === 'colorpicker') {
        t4ColorPicker.current = t4.colorPicker.create({
          inputEl: inputElRef.current,
          value,
          on: {
      ...
      t4.input.checkEmptyState(inputElRef.current);
      ...
      if (resizable) {
        t4.input.resizeTextarea(inputElRef.current);
      }
    });
  };
  ...
  const onDestroy = () => {
    ...
    if (t4Calendar.current && t4Calendar.current.destroy) {
      t4Calendar.current.destroy();
      t4Calendar.current = null;
    }
    if (t4ColorPicker.current && t4ColorPicker.current.destroy) {
      t4ColorPicker.current.destroy();
      t4ColorPicker.current = null;
    }
  };
  ...
  watchProp(colorPickerParams, (newValue) => {
    if (!t4 || !t4ColorPicker.current) return;
    extend(t4ColorPicker.current.params, newValue || {});
  });

  watchProp(calendarParams, (newValue) => {
    if (!t4 || !t4Calendar.current) return;
    extend(t4Calendar.current.params, newValue || {});
  });
  ...
  watchProp(value, (newValue) => {
    ...
    if (t4Calendar.current) {
      t4Calendar.current.setValue(newValue);
    }
    if (t4ColorPicker.current) {
      t4ColorPicker.current.setValue(newValue);
    }
  });
