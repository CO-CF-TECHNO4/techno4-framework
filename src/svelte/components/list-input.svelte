<script>
  ...
  function watchValue() {
    ...
    if (t4Calendar) {
      t4Calendar.setValue(value);
    }
    if (t4ColorPicker) {
      t4ColorPicker.setValue(value);
    }
  }

  function watchColorPickerParams() {
    if (!app.t4 || !t4ColorPicker) return;
    extend(t4ColorPicker.params, colorPickerParams || {});
  }

  function watchCalendarParams() {
    if (!app.t4 || !t4Calendar) return;
    extend(t4Calendar.params, calendarParams || {});
  }
  ...
  onMount(() => {
    t4ready(() => {
      ...
      if (type === 'datepicker') {
        t4Calendar = app.t4.calendar.create({
          inputEl,
          value,
          on: {
      ...
      if (type === 'colorpicker') {
        t4ColorPicker = app.t4.colorPicker.create({
          inputEl,
          value,
          on: {
      ...
      app.t4.input.checkEmptyState(inputEl);
      if (
      ...
      if (resizable) {
        app.t4.input.resizeTextarea(inputEl);
      }
    });
  });
