<script>
  ...
  function watchOpened(openedPassed) {
    ...
    if (!t4Popover) return;
    if (openedPassed) t4Popover.open();
    else t4Popover.close();
  }
  ...
  onMount(() => {
    ...
    t4ready(() => {
      t4Popover = app.t4.popover.create(params);
      if (opened) {
        t4Popover.open(targetEl, false);
      }
    });
  });

  onDestroy(() => {
    if (t4Popover) t4Popover.destroy();
    t4Popover = null;
  });
</script>
