  ...
  setup(props, { emit }) {
    ...
    const setTooltip = () => {
      ...
      if (!hasVisibleDataSets) {
        if (t4Tooltip && t4Tooltip.hide) t4Tooltip.hide();
        return;
      }
      ...
      if (index !== null && !t4Tooltip) {
        ...
        if (t4Tooltip && t4Tooltip.show) {
          t4Tooltip.show();
        }
        return;
      }
      if (!t4Tooltip || !t4Tooltip.hide || !t4Tooltip.show) {
        return;
      }
      if (index !== null) {
        t4Tooltip.setText(formatTooltip());
        t4Tooltip.setTargetEl(svgElRef.value.querySelector(`line[data-index="${index}"]`));
        t4Tooltip.show();
      } else {
        t4Tooltip.hide();
      }
