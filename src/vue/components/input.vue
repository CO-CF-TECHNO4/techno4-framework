...
import t4Toggle from './toggle.js';
import t4Range from './range.js';
import t4TextEditor from './text-editor.js';
  ...
  setup(props, { emit, slots }) {
    ...
    onMounted(() => {
      ...
      t4ready(() => {
        ...
        if (type === 'colorpicker') {
          t4ColorPicker = t4.colorPicker.create({
            inputEl: inputElRef.value,
            value,
            on: {
        ...
        t4.input.checkEmptyState(inputElRef.value);
        ...
        if (resizable) {
          t4.input.resizeTextarea(inputElRef.value);
        }
      });
    });
    ...
    onBeforeUnmount(() => {
      ...
      if (t4Calendar && t4Calendar.destroy) {
        t4Calendar.destroy();
        t4Calendar = null;
      }
      if (t4ColorPicker && t4ColorPicker.destroy) {
        t4ColorPicker.destroy();
        t4ColorPicker = null;
      }
    });
    ...
    watch(
      ...
      (newValue) => {
        if (!t4 || !t4ColorPicker) return;
        extend(t4ColorPicker.params, newValue || {});
      },
    );
    ...
    watch(
      ...
      (newValue) => {
        if (!t4 || !t4Calendar) return;
        extend(t4Calendar.params, newValue || {});
      },
    );
    ...
    watch(
      () => props.value,
      (newValue) => {
        if (props.type === 'range' || props.type === 'toggle' || !t4) return;
        updateInputOnDidUpdate = true;
        if (t4Calendar) {
          t4Calendar.setValue(newValue);
        }
        if (t4ColorPicker) {
          t4ColorPicker.setValue(newValue);
        }
      },
    );
